...
go build -o "$RELEASE_DIR/ha-vip" -ldflags "-X main.version=$VERSION" ./cmd/ha-vip

# Copy configuration files
cp examples/config.yaml "$RELEASE_DIR/"
cp examples/cert.pem examples/key.pem "$RELEASE_DIR/"
cp deployments/ha-vip.service "$RELEASE_DIR/"
cp deployments/setup_ha_vip.sh "$RELEASE_DIR/"
...
if [ "$ARCH" = "x86_64" ]; then
    ...
    GOOS=linux GOARCH=arm64 go build -o "$RELEASE_DIR/ha-vip-linux-arm64" -ldflags "-X main.version=$VERSION" ./cmd/ha-vip
